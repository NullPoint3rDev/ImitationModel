...
//            Row scenarioRow = scenarioSheet.getRow(1); // Пропускаем заголовок (строка 0)
//            int workersCount = 0;
//            int detailsCount = 0;
//
//            if (scenarioRow != null) {
...
//            for (int i = 1; i < productionCenterSheet.getPhysicalNumberOfRows(); i++) { // Пропускаем заголовок
//                Row row = productionCenterSheet.getRow(i);
//                if (row != null) {
//                    String id = getCellStringValue(row, 0); // ID
//                    String name = getCellStringValue(row, 1); // Имя
//                    double performance = getCellDoubleValue(row, 2); // Производительность
//                    int maxWorkersCount = getCellNumericValue(row, 3); // Макс. сотрудников
//
//                    ProductionCenter center = new ProductionCenter(i, maxWorkersCount, performance);
...
//            for (int i = 1; i < connectionSheet.getPhysicalNumberOfRows(); i++) { // Пропускаем заголовок
//                Row row = connectionSheet.getRow(i);
//                if (row != null) {
//                    String sourceCenterName = getCellStringValue(row, 0); // Источник
//                    String destCenterName = getCellStringValue(row, 1); // Назначение
//
